...
public class Utilities {
    ...
    /**
     * Return a list of SpanObject - match of regexString on text
     * @param text
     * @param regexString
     * @return
     */
...
//                    endIndex--;
                ...
                if (beginIndex < endIndex) {
                    result.add(new SpanObject(beginIndex, endIndex - 1, type, subText));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    static public List<SpanObject> getMatchGroups(String text, String regexString, String type){
        return getMatchGroups(text, regexString, type, 0);
    }

    static public String replaceString(String text, SpanObject[] spanText){
        Arrays.sort(spanText);
        ...
        while(idx < text.length()){
            if (spanId < spanText.length){
                SpanObject currentSpan = spanText[spanId];
                if (idx < currentSpan.characterStart || idx > currentSpan.characterEnd){
                    normedTextBuilder.append((text.charAt(idx)));
                } else if (idx == currentSpan.characterStart){
                    normedTextBuilder.append(currentSpan.replacement);
                }

                if (idx >= currentSpan.characterEnd){
                    spanId++;
                }
            } else{
                normedTextBuilder.append(text.charAt(idx));
